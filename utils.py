...
def jacobian(f, x, constants=[]):
    sz = shape(f)
    return tt.stacklists([grad(f[i], x) for i in range(sz[-1])])
...
class NestedMaximizer(object):
    def __init__(self, f1, vs1, f2, vs2):
        ...
        self.func1 = th.function(self.new_vs1, [-self.f1, -self.df1], givens=zip(self.vs1, self.new_vs1), on_unused_input='warn')
    ...
    def __init__(self, f, vs, g={}, pre=None, gen=None, method='bfgs', eps=1, iters=100000, debug=False, inf_ignore=np.inf):
        ...
        def f_and_df(x0):
            ...
            for _ in self.gen():
                ...
                if np.isnan(res[0]).any() or np.isnan(res[1]).any() or (np.abs(res[0]) > self.inf_ignore).any() or (
                        np.abs(res[1]) > self.inf_ignore).any():
                    continue
                if s is None:
                    s = res
                    N = 1
